...
def _repo_data_key() -> str:
    digest = hashlib.sha256(os.fsencode(str(LOCKED_ROOT))).hexdigest()[:16]
    safe_name = re.sub(r"[^A-Za-z0-9._-]+", "_", LOCKED_ROOT.name).strip("._") or "repo"
...
def _safe_graph_data_dir(repo_root: Path) -> Path:
    ...
    # Graph data lives outside the repository in the validated private runtime
    # root, so the source tree never contains generated runtime state. The
    # per-repository subdirectory is derived from LOCKED_ROOT (never from a
    # tool argument), so a prompt cannot redirect the graph to another
    # project's data or to an arbitrary path.
    runtime_data_root = _RUNTIME_ROOT / "graph-data"
    if os.path.lexists(runtime_data_root):
        metadata = runtime_data_root.lstat()
        if runtime_data_root.is_symlink() or not stat.S_ISDIR(metadata.st_mode):
            raise RuntimeError(f"refusing unsafe graph data root: {runtime_data_root}")
    else:
        runtime_data_root.mkdir(mode=0o700)
    if os.name != "nt":
        runtime_data_root.chmod(0o700)

    data_dir = runtime_data_root / _repo_data_key()
    ...
    marker = data_dir / ".yimo-pi-kit-repo"
    if not marker.exists():
        ...
        try:
            os.write(descriptor, os.fsencode(f"{LOCKED_ROOT}\n"))
        finally:
            os.close(descriptor)
    return data_dir
