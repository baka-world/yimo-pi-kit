...
- the one-time post-install/upgrade TUI hint appears once per version, distinguishes Chat Completions from Responses API, and respects `YIMO_PI_KIT_HIDE_STARTUP_HINT=1`;
...
```bash
git status --short
git tag -s v0.2.1 -m "yimo-pi-kit v0.2.1"
